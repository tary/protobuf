...
REM Pirates	C:\Program Files (x86)\Microsoft Visual Studio\2017\Enterprise\VC\Tools\MSVC\14.14.26428\INCLUDE\type_traits
...
REM disable test and protoc
powershell -Command "$text=Get-Content CMakeLists.txt ;" ^
                    "$text[27]='option(protobuf_BUILD_TESTS \"Build tests\" OFF)' ;" ^
                    "$text[29]='option(protobuf_BUILD_PROTOC_BINARIES \"Build libprotoc and protoc compiler\" OFF)' ;" ^
                    "$text| Set-Content CMakeLists.txt"